...
class InMemoryPolicyRepository(BaseRepository):
    """Dict-backed policy store for local development and testing."""
    ...
    async def list_all(
        ...
        for key, val in (filters or {}).items():
            if val is not None:
                if key == "policyholder_name__contains":
                    lower = val.lower()
                    results = [r for r in results if lower in r.get("policyholder_name", "").lower()]
                else:
    ...
    async def count(self, filters: dict[str, Any] | None = None) -> int:
        ...
        # reuse list_all logic with no pagination
