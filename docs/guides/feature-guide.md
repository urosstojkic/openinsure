...
**Foundry Agent**: The billing pipeline integrates with the **PolicyAgent** (`openinsure-policy`) which triggers billing account creation during the bind workflow. No dedicated billing Foundry agent is deployed — billing logic is handled by the backend service layer.
...
**Foundry Agent**: The **DocumentAgent** (`openinsure-document`) handles document classification and extraction via Azure Document Intelligence. Declaration/certificate generation is backend logic using policy data — no LLM call required.
...
**Foundry Agent**: Insights are generated by the backend analytics service aggregating data from all Foundry agents. When Foundry is available, the system can invoke the **SubmissionAgent** for deeper analysis. The `source` field in the response indicates whether insights came from `"system"` (computed) or `"foundry"` (AI-generated).
...
|---|---------|-------|--------|-----|----------|---------------|
| 1 | Billing Pipeline | #77 | ✅ Working | `/finance/summary`, `/billing/accounts` | `record_payment`, `create_invoice`, `get_billing_status` | PolicyAgent (bind trigger) |
...
| 7 | AI Insights | #83 | ✅ Working | `/analytics/ai-insights` | `get_ai_insights` | System + SubmissionAgent |
